    ...
    <div className="container mx-auto px-4 py-8 mt-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {explores.map((explore) => (
          <Card key={explore.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="pb-2 flex flex-row items-start space-x-4">
              <img src={explore.icon || "/placeholder.svg"} alt={explore.name} className="w-16 h-16 rounded-md" />
              <div>
                <CardTitle className="text-lg font-semibold hover:text-blue-600">
                  <Link href={`/explore/${explore.id}`}>{explore.name}</Link>
                </CardTitle>
                <div className="flex items-center mt-1 text-sm text-gray-500">
                  <div className="flex items-center">
                    {"★".repeat(Math.floor(explore.rating))}
                    {"☆".repeat(5 - Math.floor(explore.rating))}
                    <span className="ml-1">{explore.rating.toFixed(1)}</span>
                  </div>
                  <span className="mx-2">•</span>
                  <span>{explore.usageCount}次使用</span>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pb-2">
              <p className="text-gray-600 text-sm line-clamp-3">{explore.description}</p>
            ...
            <CardFooter className="pt-2 flex justify-between">
              <Button variant="outline" size="sm">
                <Link href={`/explore/${explore.id}`}>查看详情</Link>
              </Button>
