...
**Silent upstream compromise (partial).** If upstream ships a backdoored release, our Trivy scan won't catch it (scanners look for known CVEs, not zero-days). The SBOM we generate does document every package that went in, so a retroactive audit is possible. We'll often be the first to notice because a republish run exposes the compromise to more eyes — and `lastRebuiltAt` makes gaps visible when the pipeline stalls.
...
**Denial-of-service attacks on Flareo infrastructure.** If flareo.app goes down, nothing catastrophic happens — you can still verify images against Sigstore directly without us — but the catalog and web tool are unavailable.
...
**Hermetic, isolated builders.** Today's republish pipeline re-signs upstream images and records signed provenance with the upstream digest. It does not run each job on an isolated build platform that would support stronger build-provenance claims. If your admission posture requires that bar, file an issue and we'll reconsider.
...
- **Build provenance you can audit.** Signed provenance with the upstream digest recorded on every module — what was re-published, from what upstream digest, with a Rekor log entry. Verifiable independent of Flareo.
- **Catalog-level admission verdict.** The active admission policy (CVE thresholds after VEX, signature/SBOM/Rekor presence, provenance signals, composite trust score) is evaluated against every module. Verdict at `/api/v1/modules/<slug>/policy` with the per-rule breakdown. Higher-level signal than "a signature exists" — it's "a known set of rules cleared, including the ones VEX-suppressed."
...
Open a private report via [GitHub Security Advisories](https://github.com/Yolo1105/flareo/security/advisories/new). We'll respond within 72 hours and work toward coordinated disclosure.
