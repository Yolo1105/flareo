...
<aside className="mb-6 border border-warn/50 bg-warn/[0.04] p-4">
  ...
  <p className="m-0 font-body text-[13.5px] leading-[1.65] text-ink-softer">
    Third-party Dockerfile submissions are closed. This page documents how the
    retired submission pipeline worked. See the{" "}
...
Sign in at [flareo.app/signin](/signin) and open the [publish wizard](/app/publish). Four steps:
...
7. Creates the public catalog entry at `flareo.app/modules/<your-slug>`
...
3. Once upstream ships, the next republish picks it up and the module is restored. Check `lastRebuiltAt` on the module page for when that landed.
