...
Ready-to-apply policy templates live at [github.com/Yolo1105/flareo](https://github.com/Yolo1105/flareo). Three variants:
...
The Kyverno strict variant adds a live call to a **verify API URL you configure** (the shipped manifest uses the placeholder `https://REPLACE-ME.example.com/api/v1/verify`) at admission time. Point it at an instance you trust. Trade-off: adds ~200-500ms per pod creation and makes admission depend on that endpoint being reachable. The base signature-only policy has no such external dependency.
...
```
GET https://flareo.app/api/v1/modules/<slug>/policy
```
...
```
GET https://flareo.app/api/v1/modules/<slug>/vex
```
