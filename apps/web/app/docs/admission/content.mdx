...
The Kyverno strict variant adds a live call to `flareo.app/api/v1/verify` at admission time. This ensures the module's current catalog status is `verified` (not `pending` or `failing`). Trade-off: adds ~200-500ms per pod creation and makes admission depend on `flareo.app` being reachable.
