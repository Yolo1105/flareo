...
- **Admission policy verdict** — the catalog's active policy (CVE thresholds after VEX, signature/SBOM/Rekor presence, SLSA level) is evaluated against your module's signals, with the result at `/api/v1/modules/<slug>/policy`
