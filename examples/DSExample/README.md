下面提供一个完整的 README 示例，详细描述了如何复现（replicate） DSExample 项目，即同时启动 Deepseek 模型服务与聊天模块，并将其嵌入网站中。你可以将下列内容保存为 `README.md` 文件。

---

# DSExample – Deepseek Chatbox Example

本项目展示了如何使用 Docker 和 Docker Compose 部署一个基于 Deepseek 模型的聊天模块（Deepseek Chatbox），并可作为小型 Web 模块轻松嵌入到其他网站中。
...
│   └── start-model.sh         # 模型服务入口脚本，用于先启动 Ollama serve，再加载 Deepseek 模型
└── chat/
...
2. **构建并启动所有容器**
   ...
   ```
   - 该命令会构建两个容器：
     - **模型服务容器（model）**：基于 `ollama/ollama:latest` 镜像，通过自定义入口脚本启动 `ollama serve`，等待后再执行 `ollama run deepseek-r1:1.5b` 加载 Deepseek 模型，并保持容器长驻。
...
<body>
  <h1>欢迎来到我的网站</h1>
  <!-- 嵌入 Deepseek Chatbox 模块 -->
  <iframe src="http://你的服务器地址:8080" width="600" height="500" frameborder="0"></iframe>
</body>
</html>
```

请将 `http://你的服务器地址:8080` 替换为你部署 DSExample 服务的实际地址（本机测试时为 `http://localhost:8080`，部署到线上时为相应域名或 IP）。
...
- **入口脚本（model/start-model.sh）**  
  ...
  tail -f /dev/null
  ```
  该脚本确保先后台启动 `ollama serve`，等待启动后加载 Deepseek 模型，然后通过 `tail -f /dev/null` 保持容器长驻。
...
本项目提供了一个独立的 Deepseek Chatbox 模块，可通过 Docker Compose 轻松复现，并能以 iframe 方式嵌入到网站中。通过简单调整 API 接口和参数，即可根据需要扩展和定制该模块。
