...
fn version_smoke() {
    ...
    assert!(stdout.starts_with("flareo "), "unexpected version output: {stdout}");
}
...
fn help_lists_subcommands() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    for sub in ["login", "logout", "whoami", "search", "verify", "pull", "run", "update", "compose", "publish", "init"] {
        assert!(
            stdout.contains(sub),
...
fn whoami_without_token_exits_4() {
    ...
    let output = Command::new(exe)
        ...
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(4), "expected exit 4, got {:?}. stderr: {}", output.status.code(), String::from_utf8_lossy(&output.stderr));
}
